...
Este flujo utiliza únicamente Authentication y Firestore. No envía notificaciones, no inscribe asistentes y no ejecuta servicios veterinarios: publica información ficticia aprobada para la demostración. Sus reglas e índices también están pendientes de despliegue remoto. Consulte [ROLES.md](ROLES.md) y [tests/android-events.md](tests/android-events.md).
...
| `events/{eventId}` | automático | autor veterinario, dependencia, comuna, tipo, título, lugar, descripción, fecha, estado y revisión | pendiente/rechazado: autor y administrador; aprobado: lectura pública |
